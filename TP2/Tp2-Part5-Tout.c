 ...
 * Objectifs: Réaliser un chronométre sur 4 afficheurs 7 segments avec un interrupteur marche / arret et remise à zero / temps intermediaire
 * Partiellement fonctionnel
 * Limite : 
 *  - 99.99 s
 *  - Création de deux variables donc erreur lors d'une interruption et d'un arret
 *  - Lorsque le compteur est en mode intermediaire et qu'on arrete, l'état ne reviens pas en marche
 *  - Solution: Créer une variable état (0 = état initial; 1 = état marche; 2 = état arret; 3 = état intermediaire)
 * Tests: 
 *  - Etat intial : 00.00
 *  - Visualisation du compteur sur les afficheurs
 *  - Arret et marche après action sur l'interrupteur
 *  - Remise à 0 si compteur arreté
 *  - Temps intermediaire si compteur en marche
 *  - Tests qui a mené a une erreur (Cas limite)
 * 
 * Cablage: 
 *  - D0 à D7 sur a à g
    ...
    if(INT1IF) {
        ...
        if (active) {                               // Si l'état est actif
            if (!intermediaire) {                   // Et que nous sommes pas en interm. 
                ...
                centaineStock = centaine;           // Suite d'action pour stocké les valeurs actuel
...
void init() {	   
    ...
    INTCON3bits.INT1IE = 1; // Activé l'inte
