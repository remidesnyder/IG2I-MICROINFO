 ...
 * Objectifs: 
 *  - Acquisition d'une note au clavier et enregistrement d'une melodie
 * Fonctionnel
 * Limite : 
 *  - Ne prend pas en compte le temps
 *  - Limite de 10 notes maximun (modifiable facilement)
 * Tests: 
 *  - Différentes mélodie enregistrée et restituée
 * Cablage: 
 *  - C2 sur Buzzer
...
void main(void) {
    ...
    while(1) {
        ...
        if (valeurClavier >= 0 && valeurClavier <= 7){
            jouerNote(noteEnT[valeurClavier]);
            noteEnregistre[indiceNote] = valeurClavier;
            indiceNote++;
        } else if (valeurClavier == -1) {
            indiceNote = 0;
        } else if (valeurClavier == -2) {
            // Jouer
            if (!flagMelo) {
                jouerMelodie();
            }
...
int traitementClavier() {
    ...
    while(1) {
        ...
        if (C1Clavier == 1) { while(C1Clavier == 1); return -2; }
        //else if (C2 == 1) { while(C2 == 1); return '0'; }
        else if (C3Clavier == 1) { while(C3Clavier == 1); return -1; }
...
void jouerMelodie() {
    flagMelo = 1;
    int indiceNoteJoue = 0;
    
    if (indiceNote >= 1) {
        jouerNote(noteEnT[indiceNoteJoue]);
        for(int y = 1; y < indiceNote; y++) {
            while (flagV == 1);
            indiceNoteJoue++;
            jouerNote(noteEnT[noteEnregistre[indiceNoteJoue]]);
        }
    }
