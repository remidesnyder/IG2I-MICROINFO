...
// Définir les adresses des registres du DS1307
...
#define CONTROL_REG_ADDRESS 0x07 // Adresse du registre de contrôle du DS1307

// Définition des adresses I2C du DS1307
#define DS1307_ADDR_WRITE 0xD0  // Adresse I2C pour l'écriture
#define DS1307_ADDR_READ  0xD1  // Adresse I2C pour la lecture
...
while(1) 
{
 ...
 // La suite du programme principal s'effectue en boucle sans fin
 		char secondes, minutes;

        I2C_secondes_minutes(&secondes, &minutes);
...
void Init_DS1307(void) {
    // Configurer le registre de contrôle pour activer le DS1307
    i2c_start();               // Démarrer la séquence I2C
    i2c_write(DS1307_ADDRESS); // Sélectionner le DS1307 en écriture
    i2c_write(CONTROL_REG_ADDRESS); // Sélectionner le registre de contrôle
    i2c_write(0x00);           // Écrire 0x00 pour activer le DS1307
    i2c_stop();                // Arrêter la séquence I2C

    // Initialiser d'autres registres selon les besoins (exemple : initialiser l'heure à 12:00:00)
    i2c_start();               // Démarrer la séquence I2C
    i2c_write(DS1307_ADDRESS); // Sélectionner le DS1307 en écriture
    i2c_write(SEC_REG_ADDRESS); // Sélectionner le registre des secondes
    i2c_write(0x00);           // Initialiser les secondes à 00
    i2c_write(0x00);           // Initialiser les minutes à 00
    i2c_write(0x12);           // Initialiser l'heure à 12 (format 12 heures)
    i2c_write(0x01);           // Initialiser le jour à 01 (par exemple, dimanche)
    i2c_write(0x01);           // Initialiser le mois à 01 (janvier)
    i2c_write(0x21);           // Initialiser l'année à 21 (par exemple, 2021)
    i2c_stop();                // Arrêter la séquence I2C
}

// Fonction pour initialiser le DS1307
void init_DS1307() {
    // Configuration de l'heure et de la date actuelles
    // Vous devez remplacer ces valeurs par l'heure et la date actuelles
    unsigned char secondes = 0x00; // Secondes
    unsigned char minutes = 0x59;  // Minutes
    unsigned char heures = 0x12;   // Heures (format 24 heures)
    unsigned char jour = 0x05;      // Jour de la semaine (1-7, 1 = dimanche)
    unsigned char date = 0x09;      // Jour du mois
    unsigned char mois = 0x12;      // Mois
    unsigned char annee = 0x21;     // Année (les deux derniers chiffres)

    // Configuration de la sortie SQW/OUT pour une fréquence de 1 Hz
    unsigned char reg_control = 0x07;  // Registre de contrôle
    unsigned char freq_1hz = 0x10;     // Configuration de la fréquence 1 Hz

    // Démarrage de la communication I2C
    i2c_start();

    // Envoi de l'adresse I2C en écriture
    i2c_write(DS1307_ADDR_WRITE);

    // Envoi des données d'initialisation
    i2c_write(0x00);  // Adresse du registre de contrôle
    i2c_write(secondes);
    i2c_write(minutes);
    i2c_write(heures);
    i2c_write(jour);
    ...
    // Configuration de la sortie SQW/OUT
...
// Fonction pour lire les minutes et les secondes du DS1307
void read_minutes_seconds(unsigned char *minutes, unsigned char *seconds) {
    // Démarrage de la communication I2C
    i2c_start();
    ...
    // Envoi de l'adresse du registre à lire (minutes)
    i2c_write(0x01);  // Adresse du registre des minutes

    // Redémarrage de la communication I2C pour la lecture
    i2c_restart();

    // Envoi de l'adresse I2C en lecture
    i2c_write(DS1307_ADDR_READ);

    // Lecture des données
    *minutes = i2c_read(1);  // Lecture des minutes avec acquittement
    *seconds = i2c_read(0);  // Lecture des secondes sans acquittement (dernière donnée)
...
void I2C_secondes_minutes(char *secondes, char *minutes) {
    ...
    i2c_write(DS1307_ADDRESS); // Sélectionner le DS1307 en écriture
    i2c_write(SEC_REG_ADDRESS); // Sélectionner le registre des secondes
    i2c_restart();             // Redémarrer la séquence I2C
    i2c_write(DS1307_ADDRESS | 1); // Sélectionner le DS1307 en lecture
    *secondes = i2c_read(1);  // Lire les secondes avec acquittement
    i2c_stop();                // Arrêter la séquence I2C

    // Lire les minutes
    i2c_start();               // Démarrer la séquence I2C
    i2c_write(DS1307_ADDRESS); // Sélectionner le DS1307 en écriture
    i2c_write(MIN_REG_ADDRESS); // Sélectionner le registre des minutes
    i2c_restart();             // Redémarrer la séquence I2C
    i2c_write(DS1307_ADDRESS | 1); // Sélectionner le DS1307 en lecture
    *minutes = i2c_read(0);  // Lire les minutes sans acquittement
    i2c_stop();                // Arrêter la séquence I2C
}