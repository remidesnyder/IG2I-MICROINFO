 ...
 * Objectifs: Afficher une temp�rature sur un afficheur LCD (Bus I2C)
 * Fonctionnel
 * Limite : Change de temp�rature une fois sur deux
 * Tests: - Visuel
 * 
 * Cablage: 
 *  - RB3 sur LCD_E 
...
#define DS1631_ADDRESS 0b1001000
#define DS1631_ADDRESS_WRITE 0b10010000
...
void Init_DS1631(void);
void I2C_Temperature(char *donne_haut, unsigned char *donne_bas);

void convert_temperature_9(int, int);
/****************************************************************************/
...
void  __interrupt(high_priority) myHighint(void)             // High priority interrupt
{
    if (INTCONbits.TMR0IF)
	{
        ...
        i2c_start();
        i2c_write(DS1631_ADDRESS_WRITE);
        i2c_write(0x51);
        i2c_stop();
...
void Init_DS1631(void) {
    // Configurer le registre de contr�le pour activer le DS1631
    i2c_start(); 
    i2c_write(DS1631_ADDRESS_WRITE);
    i2c_write(0b10101100);  // Adresse registre de configuration
    i2c_write(0b00000001);  // Ecrit la configuration
    i2c_stop();
}
...
void I2C_Temperature(char *donne_haut, unsigned char *donne_bas) {
    ...
    i2c_write(0xAA);
    i2c_restart();
    i2c_write(DS1631_ADDRESS_READ);
    *donne_haut = i2c_read(0);
    *donne_bas = i2c_read(1);
    i2c_stop();
}

void convert_temperature_9(int xTP,int yTP) {
